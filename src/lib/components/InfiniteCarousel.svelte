<script lang="ts">
	...
	// Derived
	let duplicatedSets = $derived(Array.from({ length: setsNeeded }, (_, i) => i));
...
<div bind:this={container} class="carousel-container {className}" style:mask-image={maskStyle} style:-webkit-mask-image={maskStyle}>
	<div bind:this={track} class="carousel-track {trackClass}" style:gap="{gap}px">
		<!-- Hidden original content for measurement -->
		<div bind:this={originalContent} class="carousel-set" style:gap="{gap}px">
			{@render children()}
		</div>
